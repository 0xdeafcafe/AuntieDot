...
// ReSharper restore JoinDeclarationAndInitializer
                ...
                foreach (var tokenData in htmlDoc.DocumentNode.SelectNodes("//script").Where(script => script.InnerText.Contains("access_token") && script.InnerText.Contains("AuthenticationToken") && script.InnerText.Contains("expires_in")).Select(script => script.InnerText.Replace(" ", "").Replace("varuser=", "").TrimEnd(';')).Select(JsonConvert.DeserializeObject<Dictionary<string, string>>))
                {
